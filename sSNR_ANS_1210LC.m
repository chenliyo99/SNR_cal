...
sSNR_raw=cell(length(folder_name),8);
start_num=0;
avg_number=18;
...
for ii=1:length(folder_name)%2:2%
    for jj=1:8
        ...
        %figure(ii)
        if(jj<3)
            knum=b(3);avg_number=4;start_num=-1;%-1;
        else
            avg_number=16;
        end
        %img_tmp=cell(1,length(start_num+1:2:start_num+avg_number));
        img_tmp=zeros(kx,ky,knum,avg_number);
        ...
        img_tmp=cat(4,tmpk,tmpk2);
        
        for iTR=(start_num+3):1:(start_num+avg_number)%68%1:2%
            if(jj<3)
            ...
            img_tmp=cat(4,img_tmp,tmpk);
            
        end
        
        sSNR_raw{ii,jj}=(img_tmp);%/stdCell(img_tmp);
        a=1;
    end
end

save(['sSNR_LC20131210_' int2str(avg_number)],'sSNR_raw');
